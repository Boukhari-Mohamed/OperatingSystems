...
int cleanup_emu_file(void)
{
    ...
    int ret = remove(the_disk->emu_fname);
    if (ret < 0) {
        int errno_retd = errno;
        fprintf(stderr,
            "cleanup_emu_file for name %s failed with errno %d when "
            "calling remove(2)",
            the_disk->emu_fname, errno_retd);
    }
    return ret;
}
...
int open_emu_disk(const char *fname)
{
    ...
    the_disk->emu_fname = (char *)malloc(name_len * sizeof(char));
    strncpy(the_disk->emu_fname, fname, name_len);
