...
#ifdef PATCH_1
        unsigned int bound = hist->length == MAX_CMD_HISTORY ? hist->length - 1 : hist->length;
...
ssize_t get_command(const pvariable_t vars, pcommand_t *cmd, size_t *cmd_size)
{
    ...
    if (cmd_len != -1)
    {
        ...
        /* Copy the cmd as a working copy */
        char working_copy[*cmd_size];
        memset(working_copy, 0, *cmd_size);

        memcpy(working_copy, *cmd, *cmd_size);

        /* copy the cmd for tokenization */
        char *tmp = strdup(working_copy);
        ...
        do
        {
            ...
            else
            {
                ...
                if (get_variable(vars, &tok[1], &value) == 0)
                {
                     ...
                     */
                    size_t move_length = strlen(&working_copy[move_src]) + 1;
                    ...
                    if (cmd_len + strlen(value) - strlen(tok) >= MAX_CMD_LENGTH)
                    {
                        no_expansion = 1;
                        break;
                    }
                    memmove(&working_copy[move_dst], &working_copy[move_src], move_length);

                    /* Now we insert the value of the variable in its place. */
                    memcpy(&working_copy[offset], value, strlen(value));
        ...
        if (!no_expansion)
        {
            memcpy(*cmd, working_copy, cmd_len);
        }
    }
    return cmd_len;
}
